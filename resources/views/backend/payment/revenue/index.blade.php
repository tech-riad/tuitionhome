...
<div class="row">
    <div class="col-md-12 col-lg-12" style="margin-top: 60px">
        ...
        <div class="d-flex gap-4 flex-column flex-md-row p-3">
            <a class="text-decoration-none text-gray-800 {{ Request::is('admin/payment') ? 'active-border' : '' }}"
                href="{{route('admin.payment')}}">Revenue</a>
            <a class="text-decoration-none text-gray-800 {{ Request::is('admin/payment/due') ? 'active-border' : '' }}"
                href="{{route('admin.due.payment')}}">Due</a>
            <a class="text-decoration-none text-gray-800 {{ Request::is('admin/payment/employee') ? 'active-border' : '' }}"
                href="{{route('admin.payment.employee')}}">Employee</a>
            <a class="text-decoration-none text-gray-800 {{ Request::is('admin/payment/sms-recharge') ? 'active-border' : '' }}"
                href="{{route('admin.sms.recharge')}}">Sms Recharge</a>
            <a class="text-decoration-none text-gray-800 {{ Request::is('admin/payment/sms-recharge-due') ? 'active-border' : '' }}"
                href="{{route('admin.sms.recharge.due')}}">Sms Recharge Due</a>
            ...
            <div class="col-12 col-md-6 col-lg-9">
                <div class="bg-white shadow-lg rounded-3 p-4">
                    <p class="text-center fw-bold fs-5 mb-1">
                        {{$servicePaymentSum + $smsRechargeSum + $serviceMemberSum + $serviceboostSum + $serviceverifySum}}
                    </p>
        ...
        <div class="ps-3 mt-4" style="padding-right: 13px">
            <div class="d-flex flex-wrap flex-xl-nowrap justify-content-between flex-column flex-lg-row gap-2 gap-lg-0">
                <div class="d-flex justify-content-between gap-3 mb-3 mb-xl-0">
                    ...
                    {{-- <a href="{{ route('admin.payment.add') }}" class="btn btn-warning d-flex justify-content-center
                    align-items-center" >
                    Add Pement
                    </a> --}}
                    @if(in_array(Auth::user()->role_id, [1, 6]))
                    <button class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#addPaymentModal">
                        <i class="bi bi-currency-dollar"></i>Add Payment
                    </button>
                ...
                <div class="d-flex flex-wrap flex-md-nowrap gap-3">
                    <a href="{{route('admin.revenue.all')}}"
                        class="btn {{ Request::is('admin/revenue/serch') ? 'btn-gdark' : '' }} {{ Request::is('admin/revenue/filter') ? 'btn-gdark' : '' }} {{ Request::is('admin/all-revenue') ? 'btn-gdark' : '' }}">All</a>
                    {{-- <a href="{{route('admin.monthly.revenue')}}" class="btn btn-outline-gdark text-nowrap">
                    This Month
                    </a> --}}
                    <a href="{{route('admin.payment')}}"
                        class="btn {{ Request::is('admin/payment') ? 'btn-gdark' : '' }} d-flex justify-content-center align-items-center">
                        Set A Point
                    </a>
                    ...
                    <div class="modal-body">
                        <form id="noteForm">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="note_title" class="form-control shadow-none rounded-3"
                                    id="noteTitle" placeholder="Maximum 8 words can be given" />
                            </div>
                            <div>
                                <label for="noteDetails" class="form-label text-dark">Note Details</label>
                                <textarea name="note_details" class="form-control shadow-none rounded-3"
                                    id="noteDetails" rows="3" placeholder="Maximum 30 words can be given"></textarea>
                            </div>
                        </form>
                    </div>
                    <div class="modal-footer">
                ...
                <div class="modal-content pb-4 pt-3">
                            ...
                            <div class="d-flex align-items-center">
                                <div class="flex-grow-1 pe-4">
                                    <div class="pb-3">
                                        ...
                                        <select name="user_i" class="form-select rounded-3 shadow-none select2"
                                            aria-label="Default select" onchange="inputChange('ownership_by',this.id)"
                                            id="user_i">
                                            <option value="">Select Employee</option>
                                    ...
                                    <div class="pb-3">
                                        ...
                                        <select name="user" class="form-select rounded-3 shadow-none select2"
                                            aria-label="Default select" onchange="inputChange('verified_by',this.id)"
                                            id="user">
                                            <option value="">Select Employee</option>
                    ...
                    <div class="modal-footer d-flex justify-content-end align-items-center" style="padding-right: 27px">
                        <form action="{{route('admin.revenue.filter')}}" method="post">
                            @csrf
                            <input type="hidden" value="{{$limit}}" name="paginationLimit">
                            <div>
...
<div class="modal fade font-pop" id="addPaymentModal" tabindex="-1" aria-labelledby="addPaymentModalLabel"
    ...
    <div class="modal-dialog modal-dialog-slide-right" style="max-width: 1100px">
        <div class="modal-content pb-4 pt-3">
            <div class="modal-header" style="padding-left: 40px; padding-right: 40px">
                <h4 class="modal-title" id="addPaymentModalLabel">Revenue Add</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            ...
            <form action="{{ route('admin.payment.add') }}" method="POST">
                ...
                <div class="modal-body py-0" style="padding-left: 40px">
                    <div class="row row-cols-1 row-cols-md-2 row-cols-lg-4">
                        <!-- Transaction & Tutor ID -->
                        <div class="d-flex align-items-center">
                            <div class="flex-grow-1 pe-4">
                                <div class="pb-3">
                                    <label for="trx_id" class="form-label">Transaction ID</label>
                                    <input type="text" class="form-control shadow rounded-2" name="trx_id" id="trx_id" />
                                </div>
                                <div class="pb-3">
                                    <label for="tutor_id" class="form-label required">Tutor ID</label>
                                    <input type="number" class="form-control shadow rounded-2" required name="tutor_id"
                                        id="tutor_id" />
                                </div>
                            </div>
                            <div class="border-end mt-3" style="height: 125px"></div>
                        ...
                        <div class="d-flex align-items-center">
                            <div class="flex-grow-1 pe-4">
                                <div class="pb-3">
                                    <label for="job_offer_id" class="form-label required">Service ID</label>
                                    <input type="number" class="form-control shadow rounded-2" required name="job_offer_id"
                                        id="job_offer_id" />
                                </div>
                                <div class="pb-3">
                                    <label for="application_id" class="form-label required">Application ID</label>
                                    <input type="number" class="form-control shadow rounded-2" required name="application_id"
                                        id="application_id" />
                                </div>
                            </div>
                            <div class="border-end mt-3" style="height: 125px"></div>
                        ...
                        <div class="d-flex align-items-center">
                            <div class="flex-grow-1 pe-4">
                                <div class="pb-3">
                                    <label for="received_amount" class="form-label required">Amount</label>
                                    <input type="number" class="form-control shadow rounded-2" required name="received_amount"
                                        id="received_amount" />
                                </div>
                                <div class="pb-3">
                                    <label for="ownership_by" class="form-label text-dark required">Ownership By</label>
                                    <select required name="ownership_by" class="form-select rounded-3 shadow-none select2"
                                        aria-label="Default select" id="ownership_by">
                                        <option value="">Select Employee</option>
                                        @foreach($employees as $employe)
                                            <option value="{{$employe->id}}">{{$employe->name}}</option>
                                        @endforeach
