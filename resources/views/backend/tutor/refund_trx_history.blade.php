...
<div class="box-title bg-white" style="color: black">
    <div class="row">
        <div class="d-flex gap-4 flex-column flex-md-row p-3 mb-2">
            ...
            <a class="text-decoration-none text-gray-800 " href="{{route('admin.tutor.membership.trx.history',$id)}}">Membership Transction</a>
            <a class="text-decoration-none text-gray-800 active-border" href="{{route('admin.tutor.refund.trx.history',$id)}}">Refund Transction</a>
                ...
                <tbody>
                    @foreach ($transctions as $item)
                        <tr class="" style="vertical-align: middle">
                            ...
                            <td class="">{{ $item->job_id ?? 'n/a'}}</td>
                            <td class="">{{ $item->sending_method ?? 'n/a'}}</td>
                            <td class="">{{ $item->amount ?? 'n/a'}}</td>
                            <td class="">{{ $item->trx_id ?? 'n/a'}}</td>
                            <td class="">{{ $item->service_category ?? 'n/a'}}</td>
                            <td class="">{{ $item->paid_date ?? 'n/a'}}</td>
                        </tr>
