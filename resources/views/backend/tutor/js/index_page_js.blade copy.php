    ...
    function filterChange(colname, id){
      ...
      Object.entries(filter).forEach((entry,index) => {
        ...
        if(index==(Object.keys(filter).length-1)){
            if(key=='created_at <' || key=='country_id' || key=='city_id' || key=='created_at >' || key=='gender' || key =='tutoring_experience' || key =='religion' || key=='blood_group' || key =='method_id' || key =='group_or_major' || key=='blood_group' || key =='method_id' || key =='institute_id' || key=='category_id' || key=='curriculum_id' || key=='location_id' !! key =='education_board'){
                input+= `${key}='${value}' `;
            }
            else{
                // input+= `${key}='${value}' `;
    ...
    function btnNote(id){
      ...
      function liveChange(id){
        ...
        function paginateValue(id){
            ...
            $("#paginate_limit").val(paginate_val);

            $('#paginate_limit_form').submit(); 
