...
<main class="">
    <div class="col-md-9 ms-sm-auto col-lg-12" style="">
        ...
        <div class="d-flex gap-4 flex-column flex-md-row p-3 mb-2">
            ...
            <a class="text-decoration-none text-gray-800 active-border" href="{{route('admin.tutor.premium')}}">Premium Tutor</a>
            ...
            <a class="text-decoration-none text-gray-800" href="{{route('admin.tutor.member.count')}}">Membership Count</a>
        ...
        <div class="ps-3" style="padding-right: 13px">
            ...
            <div class="d-flex justify-content-between flex-column flex-lg-row gap-2 gap-lg-0">
                ...
                <div class="d-flex justify-content-between gap-3">
                    ...
                    @endif
                    <form action="{{ route('admin.premium.tutor.job') }}" method="post">
                        @csrf
                        <input type="hidden" name="verified" value="yes">
                        <button class="btn btn-outline btn-info" type="submit">Confirm Job-Yes</button>
                    </form>

                    <form action="{{ route('admin.premium.tutor.job') }}" method="post">
                        @csrf
                        <input type="hidden" name="verified" value="no">
                        <button class="btn btn-outline btn-danger" type="submit">Confirm Job-No</button>
                    </form>
                ...
                <table id="example1" class="table table-responsive table-hover bg-white shadow-none"
                    ...
                    <thead class="text-dark" style="border-bottom: 1px solid #c8ced3">
                        <tr>
                            ...
                            <th scope="col" class="text-nowrap">Premium Info</th>
                    ...
                    <tbody>
                        ...
                        <tr class="" style="vertical-align: middle">
                            ...
                            <td style="width: 10px" class="text-wrap">
                                ...
                                @php
                                                    $jobConfirm = App\Models\JobApplication::where('tutor_id', $tutor->id)  // Use where for single value
                                                                        ->where('current_stage', 'confirm')
                                                                        ->count('id');

                                @endphp
                            <th scope="col" class="text-nowrap">
                                <a type="button" class="text-decoration-none text-info text-nowrap @if($jobConfirm == 0)
                                text-red

                                @endif"
                                                data-bs-toggle="modal" data-bs-target="#proofModal_{{$tutor->id}}">
                                                Info
                                </a>
                            </th>
                            <!-- Proof model starts here-->
                            <div class="modal fade" id="proofModal_{{$tutor->id}}" tabindex="-1" aria-labelledby="proofModalLabel_{{$tutor->id}}" aria-hidden="true">
                                    ...
                                    <div class="modal-content">
                                        <div class="modal-body pt-4 pb-4 px-5">
                                            <div class="row row-cols-2 mt-3 border-bottom border-2 mb-4">
                                                <p class="fw-semibold">Package Name</p>

                                                @if ($tutor->is_premium == 1)
                                                    <p class="">Premium Regular</p>
                                                @elseif ($tutor->is_premium_pro == 1)
                                                    <p class="">Premium Pro</p>
                                                @elseif ($tutor->is_premium_advance == 1)
                                                    <p class="">Premium Advance</p>
                                                @endif
                                            </div>
                                            <div class="row row-cols-2 border-bottom border-2 mb-4">
                                                <p class="fw-semibold">Premium By</p>
                                                <p class="">{{$tutor->premire->name ?? ''}}</p>
                                            </div>
                                            <div class="row row-cols-2">
                                                <p class="fw-semibold">Premium Date</p>
                                                <p class="">{{$tutor->premium_date ?? ''}}</p>

                                            </div>
                                            <div class="row row-cols-2">
                                                <p class="fw-semibold">Job Confirm</p>

                                                <p class="">{{ $jobConfirm }}</p>  <!-- Display the confirmed jobs count -->
                                            </div>
                                            <div class="row row-cols-2">
                                                <p class="fw-semibold">Premium Expire </p>
                                                <p class="">{{ $tutor->premium_expire ?? '' }}</p>  <!-- Display premium date -->
                                            </div>
    ...
    <form action="{{route('admin.tutor.filter')}}" method="post">
        @csrf
        <input type="hidden" value="1" name="is_premium">
